...
export async function syncCricbuzzScores(matchId: number) {
  ...
  if (!match?.cricbuzz_match_id) return { error: 'No Cricbuzz Match ID set.' };

  const scrapedData = await scrapeAndCalculatePoints(match.cricbuzz_match_id);
  if (!scrapedData) return { error: 'Failed to scrape scores.' };

  const scrapedScores = scrapedData.playerPoints; // Use the nested points object

  const { data: players } = await supabase.from('players').select('id, name, cricbuzz_name').in('team', [match.team1, match.team2]);
  if (!players) return { error: 'Failed to fetch players.' };
...
export async function toggleAutomation(matchId: number, isAutomated: boolean) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (user?.email?.toLowerCase() !== ADMIN_EMAIL.toLowerCase()) return { error: 'Unauthorized' }

  const { error } = await supabase.from('matches').update({ is_automated: isAutomated }).eq('id', matchId)
  if (error) return { error: error.message }
